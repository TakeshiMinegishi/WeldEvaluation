...
private:
	...
	int			m_ScanDataHolizontalSize;	///< スキャンデータ水平方向サイズ（撮影データからの縮退後のサイズ）
	int			m_ScanDataVerticalSize;		///< スキャンデータ垂直方向サイズ（撮影データからの縮退後のサイズ）
...
public:
	...
	UINT GetScanDataVerticalResolution(void);
	bool SetScanDataVerticalResolution(UINT resolution);
	UINT GetScanDataHorizontalResolution(void);
	bool SetScanDataHorizontalResolution(UINT resolution);
